...
// Don't question it
export async function queryGrades(
	token: string,
	studentId: string,
	date: string
	...
	for (let day of resp.weekGradesTable.days) {
		for (let grade of day.lessonGrades) {
			ret.push({
				grades: grade.grades,
				lessonNumber: grade.sequenceNumber,
...
export async function queryHomework(
	...
	let endpoint = `/homework?studentId=${studentId}&date=${date}`;
...
export async function querySchedule(
	...
	const resp = await queryEndpoint(
		`/schedule?studentId=${studentId}${date ? '&date=' + date : ''}`,
		token
	);
